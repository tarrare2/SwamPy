            ...
            try:
                ...
                if game.platform.lower() == 'psv' and self.psv_helper and game.rom_id:
                    ...
                    if is_pkg:
                        ...
                        # Decrypt PKG to ZIP if enabled (and not already installed)
                            ...
                            if zip_path:
                                ...
                                # Note: We do NOT auto-install the ZIP; it's just a decrypted archive
                        ...
                        if path.lower().endswith('.zip'):
                            # We never auto-install ZIPs – they are just downloaded files
                            installed = False
...
class GameCard(QFrame):
    ...
    def refresh_theme(self):
        ...
        self.title_label.setStyleSheet("font-weight: bold;")  # this is palette-independent
...
class SettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.decrypt_psv_cb = QCheckBox("Automatically decrypt PSVita's PKG files (using pkg2zip)")
        self.decrypt_psv_cb.setChecked(self.settings.get("decrypt_psv_pkg", True))
        self.decrypt_psv_cb.setToolTip("When enabled, downloaded .pkg files for the PSVita will be decrypted to .zip using pkg2zip")
        download_layout.addWidget(self.decrypt_psv_cb)
        # Vita3K settings
        self.auto_install_cb = QCheckBox("Automatically install PSV games in Vita3K")
        self.auto_install_cb.setChecked(self.settings.get("auto_install_vita3k", False))
        self.auto_install_cb.setToolTip("After decrypting a PSV PKG, run Vita3K to install the resulting package. It will run the game as well, unfortunately.")
        download_layout.addWidget(self.auto_install_cb)
        self.delete_after_install_cb = QCheckBox("Delete original file after Vita3K installation")
        self.delete_after_install_cb.setChecked(self.settings.get("delete_after_vita3k_install", False))
        self.delete_after_install_cb.setToolTip("If enabled, the downloaded PKG or ZIP file will be deleted after successful installation to Vita3K.")
        download_layout.addWidget(self.delete_after_install_cb)
        ...
        exe_browse.clicked.connect(self.browse_vita3k)
        exe_layout.addWidget(self.vita3k_edit)
        exe_layout.addWidget(exe_browse)
        download_layout.addLayout(exe_layout)
    ...
    # UI Update - Shows server state, no signals needed
    ...
    def save(self):
        print(f"SettingsDialog.save: decrypt_psv_cb.isChecked() = {self.decrypt_psv_cb.isChecked()}")
        settings = {
            ...
            "decrypt_psv_pkg": self.decrypt_psv_cb.isChecked(),
            "vita3k_path": self.vita3k_edit.text(),
            "auto_install_vita3k": self.auto_install_cb.isChecked(),
            "delete_after_vita3k_install": self.delete_after_install_cb.isChecked(),
...
class MainWindow(QMainWindow):
    ...
    def _create_cards_from_games(self):
        ...
        for i, game in enumerate(self.current_games):
            ...
            # card.clicked is not used anymore, but kept for compatibility
    ...
    def _on_card_left_clicked(self, game: Game, modifiers: Qt.KeyboardModifiers):
        ...
        if modifiers & Qt.ControlModifier:
            ...
            self.last_selected_index = index   # or keep the original anchor? We'll update to current
        else:
            ...
            self.cards[index].selected = True
            self.last_selected_index = index
        # Optionally update the download action (if you want to show number selected)
    ...
    def _apply_theme(self, theme_name):
        ...
        # 3. Force all widgets to re‑evaluate the new style/palette
        for widget in app.allWidgets():
            if not widget:
                continue
            # Temporarily remove stylesheet to force re‑evaluation of palette(role)
